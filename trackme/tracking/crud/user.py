...
async def auth_user(user: UserInput, 
    ...
    async with async_session() as db:
        ...
        if verify_user is not None:
            existing_token = await db.query(UserActivityModel)\
...
async def _get_user(db: AsyncSession, user: UserInput) -> Optional[UserOutput]:
    try:
        existing_user = await db.query(UserModel).filter(UserModel.name == user.name).first()
...
async def get_user_by_token(db: AsyncSession, token: str) -> Optional[int]:
    user_id = await db.query(UserActivityModel.user_id)\
    .filter(UserActivityModel.token == token)\
    .first()
    return user_id[0]
